...
$.BootstrapTable = class extends $.BootstrapTable {
  ...
  resetView (...args) {
    ...
    $(window).resize(() => this.renderStickyHeader())
  }
  ...
  renderStickyHeader () {
    ...
    function getRightOffset(leftOffset, $table) { //add this
      ...
      if (typeof (leftOffset) === 'string') {
        leftOffsetWithoutUnit = parseFloat(leftOffset.match(/[+-]?\d+(\.\d+)?/g))
      }

      return ($(window).width() - leftOffsetWithoutUnit - $table.parents('.fixed-table-container').width()) + 'px'
    }
  }
